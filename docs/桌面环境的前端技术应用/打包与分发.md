...
2. 如果有则检查其内容
    1. 其版本与当前启动应用的版本是否一致,不一致则询问用户是否用当前的`.desktop`文件替换原有,用户同意就替换
    2. 其Exec指定的启动路径与当前的应用路径是否一致, 不一致则询问用户是否用当前的应用路径替换Exec指定的启动路径,用户同意就替换
    3. 检查用户`~/.local/share/icon`目录下是否有当前应用对应的图标,没有就复制一份过去
3. 如果没有则:
...
// 获取环境变量`APPDIR`,只有在AppImage中运行时这个环境变量才会有值且为运行时的项目根目录挂载位置
