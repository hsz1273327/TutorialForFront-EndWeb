...
3. 修改`vite.config.js`
    ...
    import manifest from './manifest.json' assert { type: 'json' } // <=新增
...
执行`npm run dev`后我们会在项目根目录得到`dist`文件夹,这就是插件文件夹了.我们就可以拿这个文件夹在chrome中加载为插件.调试模式中命令行并不会关闭,它会根据操作打印日志,如果关闭这个命令行,插件也会失效.
...
在接下来的篇幅中我们用这套方法实地的构造一个简单的ollama调用插件用于做本地翻译,让它实现如下功能

+ popup页,可以用于查看ollama当前维护的大模型,以及对应功能的prompt
+ 右键菜单,用于框定内容然后用ollama提供的模型进行翻译

这个项目并不是一个玩具项目,我们会单独放在仓库[hsz1273327/chrome-ollama-translate](https://github.com/hsz1273327/chrome-ollama-translate)中
