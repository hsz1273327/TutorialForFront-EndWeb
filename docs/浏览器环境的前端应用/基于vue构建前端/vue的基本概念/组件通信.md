...
子组件向父组件传递消息最常见的方式是使用[自定义事件传递](https://v3.cn.vuejs.org/guide/component-custom-events.html#%E5%AE%9A%E4%B9%89%E8%87%AA%E5%AE%9A%E4%B9%89%E4%BA%8B%E4%BB%B6),和`props`类似,我们需要在组件中先用声明有哪些自定义事件类型,然后子组件就可以使用`emit(事件类型:String[,消息:Any])`函数发出一个事件.父组件则使用`v-on:事件类型=事件处理函数或表达式`监听这个事件处理消息.
