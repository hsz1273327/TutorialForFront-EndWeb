...
tailwind是前端流行的原子化CSS框架,它提供了许多已经封装好的细粒度class(原子class)让我们可以几乎不手写css,仅调用这些类就可以设置样式.

nativescript官方提供了插件[@nativescript/tailwind](https://github.com/NativeScript/tailwind)让它可以在移动端也生效.
